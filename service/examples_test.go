...
func ExampleServer() {
	...
	svr.ListenAndServe("tcp://:1883") // nolint: errcheck
}
...
func ExampleClient() {
	...
	c.Connect("tcp://127.0.0.1:1883", msg) // nolint: errcheck

	// Creates a new SUBSCRIBE message to subscribe to topic "abc"
	submsg := message.NewSubscribeMessage()
	submsg.AddTopic([]byte("abc"), 0) // nolint: errcheck
	...
	c.Subscribe(submsg, nil, nil) // nolint: errcheck

	// Creates a new PUBLISH message with the appropriate contents for publishing
	pubmsg := message.NewPublishMessage()
	pubmsg.SetTopic([]byte("abc")) // nolint: errcheck
	pubmsg.SetPayload(make([]byte, 1024))
	pubmsg.SetQoS(0) // nolint: errcheck

	// Publishes to the server by sending the message
	c.Publish(pubmsg, nil) // nolint: errcheck
